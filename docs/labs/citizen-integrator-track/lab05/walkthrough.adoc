...
. Log in to link:{3scale-admin-url}[3scale Admin, window="_blank"] web console using `{user-username}` and password: `{user-password}`. Click on *Log in*.
+
...
. Go back to your _Accounts > Listing_  and navigate to *Developers* to activate the new account.
+
